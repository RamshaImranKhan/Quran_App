...
const UrduTranslation = () => {
  ...
  const fetchSurahData = async () => {
    try {
      ...
      // Fetch translations
      const translationResponse = await axios.get(`https://api.alquran.cloud/v1/surah/${surahNumber}/editions/quran-uthmani,ur.ahmedali`);
      setSurah(translationResponse.data.data);
      setLoading(false);
    } catch (err) {
      console.error('Error fetching surah:', err);
      setError('سورہ کی معلومات حاصل کرنے میں خرابی: ' + err.message);
      setLoading(false);
    }
  };
    ...
    return (
      ...
      >
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>سورہ {surahNumber} لوڈ ہو رہی ہے...</Text>
        </View>
      </LinearGradient>
    );
  }
  ...
  if (!surah || !surahInfo) {
    return (
      ...
      >
        <View style={styles.errorContainer}>
          <Text style={styles.errorText}>اس سورہ کے لیے کوئی ڈیٹا دستیاب نہیں ہے</Text>
        </View>
      </LinearGradient>
    );
  }
        ...
        <View style={styles.surahInfoContainer}>
          ...
          <View style={styles.detailsGrid}>
            <View style={styles.detailItem}>
              <Text style={styles.detailLabel}>نزول</Text>
              <Text style={styles.detailValue}>{surahInfo.revelationType === 'Meccan' ? 'مکی' : 'مدنی'}</Text>
            </View>
            <View style={styles.detailItem}>
              <Text style={styles.detailLabel}>آیات</Text>
              <Text style={styles.detailValue}>{surahInfo.numberOfAyahs}</Text>
            </View>
            <View style={styles.detailItem}>
              <Text style={styles.detailLabel}>سورہ نمبر</Text>
          ...
          <View key={ayah.number} style={styles.ayahContainer}>
            ...
            <View style={styles.translationContainer}>
              <Text style={styles.translationText}>
                {surah[1]?.ayahs[index]?.text || 'ترجمہ دستیاب نہیں'}
              </Text>
